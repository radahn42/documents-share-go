...
func (r *documentRepository) Create(ctx context.Context, doc *entities.Document) error {
	query := `INSERT INTO documents (name, owner_id, mime, is_file, is_public, file_path, json_data, "grant")
...
func (r *documentRepository) GetByOwner(ctx context.Context, filter *entities.DocumentFilter) ([]*entities.Document, error) {
	...
	if filter.OwnerID != "" {
		if filter.RequestingUserID != "" && filter.RequestingUserID != filter.OwnerID {
			query += fmt.Sprintf(` AND owner_id = $%d AND (is_public = true OR $%d = ANY("grant"))`, argIndex, argIndex+1)
	...
	if filter.Key != "" && filter.Value != "" {
		...
		case "mime":
			...
			argIndex++
		case "public":
			publicBool, err := strconv.ParseBool(filter.Value)
			if err != nil {
				return nil, fmt.Errorf("invalid value for public: %s", filter.Value)
			}
			query += fmt.Sprintf(" AND is_public = $%d", argIndex)
			args = append(args, publicBool)
			argIndex++
		default:
...
func (r *documentRepository) CheckAccess(ctx context.Context, docID string, userID string) (bool, error) {
	...
	var hasAccess bool
	row := r.pool.QueryRow(ctx, query, docID, userID)
